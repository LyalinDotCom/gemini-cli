...
type PlanChoice = 'execute' | 'execute_clean' | 'save' | 'refine' | 'cancel';
...
export function PlanCompletionDialog({ request }: PlanCompletionDialogProps) {
  ...
  const OPTIONS: Array<RadioSelectItem<PlanChoice>> = [
    {
      label: 'Execute (default) - Start with plan context',
      value: 'execute',
      key: 'execute',
    },
    {
      label: 'Execute (clean context) - Start fresh with the plan only',
      value: 'execute_clean',
      key: 'execute_clean',
    },
    {
