...
describe('UiTelemetryService', () => {
  ...
  describe('API Response Event Processing', () => {
    it('should process a single ApiResponseEvent', () => {
      ...
      const metrics = service.getMetrics();
      expect(metrics.models['gemini-3-pro-preview']).toEqual({
        api: {
          totalRequests: 1,
          totalErrors: 0,
          totalLatencyMs: 500,
    ...
    it('should handle ApiResponseEvents for different models', () => {
      ...
      const metrics = service.getMetrics();
      expect(metrics.models['gemini-3-pro-preview']).toBeDefined();
      expect(metrics.models['gemini-3-flash-preview']).toBeDefined();
      expect(metrics.models['gemini-3-pro-preview'].api.totalRequests).toBe(1);
      expect(metrics.models['gemini-3-flash-preview'].api.totalRequests).toBe(
        1,
      );
      expect(service.getLastPromptTokenCount()).toBe(0);
    });
